...
int Rating::compare(Object* obj){
        Rating *r = dynamic_cast<Rating *>(obj);
        if(r==NULL){
                cerr<<"Rating:Error:compare: Bad cast, original type is not Rating\n";
                return -2;
        }
        else{
                if(this->score == r->score){
                        if(this->comment == r->comment) return 0;
                        else if(this->comment < r->comment) return 1;
                        else return -1;
                }
                else if(this->score < r->score) return 1;
                else return -1;
        }
}
...
void Rating::read(){
	cout<<"rating: "; cin>>score;
	cout<<"comment: "; cin>>comment;
}
